...
## Pair status (Stage 5, step 3)

get_payment_status accepts a PaymentPair produced by match_unique_payments. It uses amounts_match to return matched for equal amounts and amount_mismatch for unequal amounts. Any difference, including one cent, produces amount_mismatch.

```python
from reconciliation.engine import get_payment_status

for pair in pairs:
    print(pair.expected.payment_reference, get_payment_status(pair))
```
